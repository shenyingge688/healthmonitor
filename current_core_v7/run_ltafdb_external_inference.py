"""
Run frozen Round5 ensemble inference on the preselected LTAFDB pilot.
...
PROTOCOL_VERSION = "ltafdb-pilot-v1"
...
def build_protocol(manifest_path, selected_leads_path, decision_path):
    ...
    return {
        ...
        "pilot_manifest_path": str(Path(manifest_path).resolve()),
...
def freeze_protocol(output_dir, manifest_path, selected_leads_path, decision_path):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    protocol_path = output_dir / "external_evaluation_protocol.json"
    expected = canonical_json(
        build_protocol(manifest_path, selected_leads_path, decision_path)
    )
...
def build_record_features(ecg, maximum_trajectories=None):
    ...
    for index, window_indices in enumerate(trajectory_indices):
        available = [
        ...
        trajectory = fast_trajectory_features(concatenated)
        rr_last[index, :4] = feature4[int(window_indices[-1])]
        rr_last[index, 4:] = trajectory
        ...
        if index < 3 and len(concatenated) > 0:
            ...
            if not np.allclose(legacy, trajectory, rtol=0.0, atol=1e-7):
                raise RuntimeError(
...
def infer_record(
    ...
    features = build_record_features(ecg, maximum_trajectories)
...
def main():
    ...
    parser.add_argument("--trajectory-batch-size", type=int, default=128)
    ...
    manifest = manifest.merge(
        leads[["record_id", "selected_lead_index", "record_quality_pass"]],
        on="record_id",
        how="left",
        validate="one_to_one",
    )
    if not manifest["record_quality_pass"].all():
        raise RuntimeError("frozen manifest contains a failed signal record")
    if args.records:
        requested = [
            item.strip().zfill(2)
