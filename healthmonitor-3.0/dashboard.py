...
# ==========================================
clinical_cases = {
    "100": {"bed_label": "档案 100 · 正常窦性心律",  "desc": "Class 0 — 正常窦性心律，偶发早搏，基线平稳，无持续心律失常。"},
    "119": {"bed_label": "档案 119 · 频发室性早搏",  "desc": "Class 1 — 频发室性早搏 (PVC)，含二联律/三联律/成对室早。"},
    "201": {"bed_label": "档案 201 · 阵发性心房颤动", "desc": "Class 2 — 阵发性房颤 (Paroxysmal AFib)，RR 间期绝对不齐，P 波消失。"},
    "207": {"bed_label": "档案 207 · 室速 / 室扑",    "desc": "Class 3/4 — 短阵室性心动过速 (VT) 与心室扑动 (VFl) 交替发作。"},
    "209": {"bed_label": "档案 209 · 房速 / 房扑 / 室上速", "desc": "Class 5 — 阵发性房性心动过速 (AT) / 心房扑动 (AFl) / 室上速 (SVT)。"},
...
def load_sim_data(rec_id):
    ...
    # 与 V16 共用 MIT-BIH 数据
    data_dir = os.path.join(os.path.dirname(base_dir), 'data', 'mitdb')
...
if st.sidebar.button("🔴 启动预警引擎", use_container_width=True):
    ...
    while True:
        ...
        for cls_idx, prob in sorted_probs:
            ...
            bar_color = accent_color if cls_idx == pred_class else "rgba(255,255,255,0.25)"
            ...
            distribution_html += f"<span style='width: 160px; font-weight: {font_weight};'>{cls_name}</span>"
            distribution_html += f"<div style='flex-grow: 1; background: rgba(0,0,0,0.3); border-radius: 4px; margin: 0 12px; height: 8px;'>"
            distribution_html += f"<div style='width: {bar_width:.1f}%; background: {bar_color}; height: 100%; border-radius: 4px;'></div>"
            distribution_html += f"</div>"
            distribution_html += f"<span style='width: 55px; text-align: right;'>{prob*100:.1f}%</span>"
            distribution_html += f"</div>"
