...
DATA_DIR = os.path.join(SHARED_ROOT, 'data', 'mitdb')
...
# ==========================================
# 模块 3：本地数据校验 (与 V16 共用)
# ==========================================
print(f"📂 校验本地 MIT-BIH 数据: {DATA_DIR}")
dat_files = [f for f in os.listdir(DATA_DIR) if f.endswith('.dat')] if os.path.isdir(DATA_DIR) else []
if not dat_files:
    print(f"❌ 未发现 MIT-BIH 数据，请先运行 V16 的 build_dataset_factory.py 下载。")
    exit(1)
print(f"✅ 本地数据就绪，共 {len(dat_files)} 条记录。")

# 定义多分类映射字典
def get_class_from_annotation(symbol, aux_note):
    # 优先匹配节律级恶性事件 (aux_note)
    if aux_note:
        note = aux_note.upper().replace('\x00', '')
...
def build_balanced_dataset(record_list, name, risk_threshold=20):
    ...
    for rec in pbar:
        try:
            ...
            # AFib/AT 节律窗口扩展: aux_note 仅标记起始点
            for cls_val, win_sec in [(2, 60), (5, 15)]:
                cls_mask = anno_classes == cls_val
                if cls_mask.any():
                    for i in np.where(cls_mask)[0]:
                        end_s = anno_idx[i] + win_sec * TARGET_FS
                        spread = (anno_idx > anno_idx[i]) & (anno_idx < end_s)
                        # 仅覆盖 Normal (0)，不覆盖 PVC/VF/VT
                        anno_classes[spread & (anno_classes == 0)] = cls_val
