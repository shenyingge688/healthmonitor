...
def download_dat(
    ...
    for attempt in range(1, int(attempts) + 1):
        downloaded_bytes = temporary.stat().st_size if temporary.exists() else 0
        ...
        try:
            ...
            print(
                ...
                f"{attempt}/{attempts}, retained "
                f"{retained_bytes}/{expected_bytes} bytes: {exc}"
            )
            time.sleep(wait_sec)
...
def summarize_cached_signals(records_dir, manifest_records, headers):
    complete_count = 0
    complete_bytes = 0
    in_order_complete = 0
    ...
    for record in manifest_records:
        expected_bytes = expected_dat_bytes(headers[record])
        dat_path = Path(records_dir) / f"{record}.dat"
        part_path = Path(records_dir) / f"{record}.dat.part"
        actual_bytes = dat_path.stat().st_size if dat_path.exists() else 0
        complete = dat_path.exists() and actual_bytes == expected_bytes
        if complete:
            complete_count += 1
            complete_bytes += actual_bytes
    ...
    return {
        "complete_signal_files_in_shared_cache": int(complete_count),
        "complete_signal_bytes": int(complete_bytes),
        "manifest_records_fully_downloaded_in_order": int(in_order_complete),
...
def write_progress_checkpoint(
    ...
    cache_summary = summarize_cached_signals(
        records_dir, manifest_records, headers
    )
    partials = cache_summary["partial_records"]
    primary_partial = partials[0] if partials else {}
    progress = {
        "status": status,
        "updated_local": time.strftime("%Y-%m-%d %H:%M:%S"),
        "manifest_records": int(len(manifest_records)),
        "manifest_records_fully_audited_in_order": int(audited_count),
        "current_record": current_record,
        "current_step": current_step,
        "last_completed_record": last_completed_record,
        "last_download_status": last_download_status,
        "partial_record": primary_partial.get("record_id"),
        "partial_bytes_retained": int(
            primary_partial.get("partial_bytes", 0)
        ),
        "exact_byte_count_required_before_promotion": True,
    ...
    progress_path = Path(output_dir).with_name(
        f"{Path(output_dir).name}_progress.json"
    )
...
def main():
    ...
    manifest_records = manifest["record_id"].tolist()
    ...
    for index, record in enumerate(manifest["record_id"], start=1):
        try:
            write_progress_checkpoint(
                ...
                headers,
                records_dir,
                audited_count=index - 1,
                status="running",
                current_record=record,
                current_step="copy_metadata_download",
            )
            ...
            write_progress_checkpoint(
                ...
                current_step="audit_signal_and_preprocessing",
            )
    ...
    write_progress_checkpoint(
        ...
        headers,
        records_dir,
        audited_count=len(manifest_records),
        status="complete",
        last_completed_record=manifest_records[-1],
        current_step="outputs_written",
    )
