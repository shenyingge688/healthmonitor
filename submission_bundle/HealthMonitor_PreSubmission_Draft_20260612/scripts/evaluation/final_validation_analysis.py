...
def compute_metrics(
    ...
    if target_fut_soft is not None:
        soft = np.asarray(target_fut_soft, dtype=np.float64)
        probs = np.asarray(probs_fut, dtype=np.float64)
        if soft.shape != probs.shape:
            raise ValueError(
                f"soft target shape {soft.shape} != probability shape {probs.shape}"
            )
        soft_sum = soft.sum(axis=1, keepdims=True)
        soft = soft / np.clip(soft_sum, 1e-12, None)
        probs = np.clip(probs, 1e-8, 1.0)
        probs = probs / probs.sum(axis=1, keepdims=True)
        soft_cross_entropy = -np.sum(soft * np.log(probs), axis=1)
        soft_entropy = -np.sum(
            np.where(soft > 0, soft * np.log(np.clip(soft, 1e-12, 1.0)), 0.0),
            axis=1,
        )
        abnormal_target_mass = 1.0 - soft[:, 0]
        abnormal_pred_mass = 1.0 - probs[:, 0]
        abnormal_error = abnormal_pred_mass - abnormal_target_mass
        out.update({
            "future_soft_cross_entropy": float(np.mean(soft_cross_entropy)),
            "future_soft_kl": float(
                np.mean(soft_cross_entropy - soft_entropy)
            ),
            "future_soft_brier": float(
                np.mean(np.sum((probs - soft) ** 2, axis=1))
            ),
            "future_abnormal_mass_mae": float(
                np.mean(np.abs(abnormal_error))
            ),
            "future_abnormal_mass_rmse": float(
                np.sqrt(np.mean(abnormal_error ** 2))
            ),
            "future_abnormal_mass_bias": float(np.mean(abnormal_error)),
        })
...
def patient_bootstrap(outputs_df, n_boot=1000, seed=42):
    ...
    target_fut_soft = outputs_df[
        [f"target_fut_soft_{k}" for k in CLASS_KEYS]
    ].to_numpy()
...
def patient_robustness_analysis(outputs_df, out_dir):
    ...
    (
        ...
        transition,
        target_fut_soft,
    ) = _arrays_from_outputs_df(outputs_df)
    observed = compute_metrics(
        target_cur,
        target_fut,
        probs_cur,
    ...
    for patient in patients:
        ...
        pc, pf, prc, prf, tr, pfs = _arrays_from_outputs_df(patient_df)
        row = compute_metrics(pc, pf, prc, prf, tr, pfs)
        ...
        pc, pf, prc, prf, tr, pfs = _arrays_from_outputs_df(keep_df)
        loo = compute_metrics(pc, pf, prc, prf, tr, pfs)
